...
import io.github.setl.{Setl, SparkSessionBuilder}
...
class SimplePipelineOptimizerSuite extends AnyFunSuite {
  ...
  test("PipelineOptimizer should optimize a pipeline") {
    ...
    pipeline.deliverableDispatcher.getRegistry.values.foreach(println)
  ...
  test("Pipeline optimise should opt") {
    ...
    val pipeline = context
      ...
      .addStage(classOf[F5])
    pipeline.describe()
    assert(pipeline.stages.length === 7)
    ...
    assert(pipelineOpt.stages.length === 2)
