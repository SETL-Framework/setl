...
import io.github.setl.storage.connector.{CSVConnector, Connector, FileConnector}
import io.github.setl.storage.repository.SparkRepository
import io.github.setl.storage.{Condition, ConnectorBuilder, SparkRepositoryBuilder}
...
class SetlSuite extends AnyFunSuite with PrivateMethodTester with Matchers {
  ...
  test("getSparkRepository should return a registered repository without changing it") {
    ...
    import io.github.setl.SetlSuite.TestGetRepository
  ...
  test("Setl should handle delivery annotated method with deliveryID") {
    ...
    val factory = new io.github.setl.SetlSuite.FactoryWithConnectorDeliveryMethod
